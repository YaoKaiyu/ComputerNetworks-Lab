...
void mospf_run() {
    ...
    pthread_create(&db, NULL, dump_database, NULL);
}
...
void send_mospf_lsu() {
    ...
    struct mospf_lsa    * lsa   = NULL, * lsa_packets = NULL;
    char * packet = NULL;
    iface_info_t * iface = NULL, * iface_q = NULL;
    mospf_nbr_t  * nbr_entry = NULL, * nbr_q = NULL;
    ...
    instance->sequence_num = nbr_num;
    // get informations of all neighbours into lsa
    lsa_packets = (struct mospf_lsa *)malloc(nbr_num * MOSPF_LSA_SIZE);
    if(!lsa_packets){printf("lsa_packets malloc error.\n"); exit(-1);}
    if(!list_empty(&(instance->iface_list))){
        struct mospf_lsa * lsa_idx = lsa_packets;
        list_for_each_entry_safe(iface, iface_q, &(instance->iface_list), list){
            if(!list_empty(&(iface->nbr_list))){
                list_for_each_entry_safe(nbr_entry, nbr_q, &(iface->nbr_list), list){
        ...
        list_for_each_entry_safe(iface, iface_q, &(instance->iface_list), list){
            if(!list_empty(&(iface->nbr_list))){
                list_for_each_entry_safe(nbr_entry, nbr_q, &(iface->nbr_list), list){
                    ...
                    // init ether header
                    eth = (struct ether_header *)packet; 
                    memcpy(eth->ether_shost, iface->mac, ETH_ALEN); // dhost will be set in ip_forward_packet
                    ...
                    memcpy((void*)lsa, (void*)lsa_packets, nbr_num * MOSPF_LSA_SIZE);
...
void handle_mospf_lsu(iface_info_t *iface, char *packet, int len) {
    ...
    if((lsu->ttl--) > 0){
        ...
        struct ether_header * out_eth   = NULL;
        struct iphdr        * out_ip    = NULL;
        struct mospf_hdr    * out_mospf = NULL;
        iface_info_t * iface = NULL, * iface_q = NULL;
        mospf_nbr_t  * nbr_entry = NULL, * nbr_q = NULL;
        // for each iface and its nbr   
        // forward the packet  
        list_for_each_entry_safe(iface, iface_q, &(instance->iface_list), list){
            if(!list_empty(&(iface->nbr_list))){
                list_for_each_entry_safe(nbr_entry, nbr_q, &(iface->nbr_list), list){
                    if(nbr_entry->nbr_ip != ntohl(ip->saddr) && nbr_entry->nbr_id != ntohl(mospf->rid)) { // avoid sending packet back to source
                        ...
                        // change ether header
                        out_eth = (struct ether_header *)out_packet;
                        memcpy(out_eth->ether_shost, iface->mac, ETH_ALEN);
                        // change ip header
